function validation() {
    if (dept() && fname() && id() && email() && research()) {
        return true;
    }
    else {
        return false;
...
function research() {
    var rs1 = document.getElementById("rsch");
    var rs2 = document.getElementById("rsch2");
    if (rs1.checked == true || rs2.checked == true) {
...
{
    var teacher = document.getElementById("search").value;
...
obj.open("GET","http://localhost/WT/controller/techrdb_process.php?Teacher=" + teacher,true);
obj.send();
}